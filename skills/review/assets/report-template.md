...
**审查范围**: `<scope>`（二选一：`指定提交/修订` / `工作区未提交变更`）

（以下仅当审查范围为 **指定提交/修订** 时填写）
- **变更 ID（commit/revision）**: `<commit_id_or_revision>`
- **提交时间**: `<commit_date>`
- **作者**: `<author>`
- **提交消息**: `<commit_message>`

（当审查范围为 **工作区未提交变更** 时，不填上述四项，仅保留「审查范围：当前工作区未提交的变更（Git: 已暂存 + 未暂存；SVN: 工作副本变更）」）
...
> **适用说明**：本节仅当审查范围为 **指定提交/修订** 时填写（有 commit message 可对照）。审查范围为 **工作区未提交变更** 时本节**可省略**；若用户在前文说明了修改目的，可在此简要写「修改目的验证」并评价变更是否贴合目的。

### 5.1 提交描述分析（仅指定提交/修订时）
